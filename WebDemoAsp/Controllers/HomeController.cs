...
namespace WebDemoAsp.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            var result = GetTianjinPostMethod(Request.Form.Get("vcode"));
...
fpcy.type=0"));
        ...
        private static string GetHenanMethod()
        {
            ...
            wbx.Headers.Add("Accept-Language", "zh-CN,zh;q=0.8,en;q=0.6,zh-TW;q=0.4");
            Encoding enc = Encoding.GetEncoding("UTF-8");
            wbx.DownloadFile(StateTaxationConstants.HenanGet, @"D:/codeHenan.png");
        ...
        private static string GetHenanPostMethod()
        {
            // GET
            WebClient wbx = new WebClient();
            //HttpClient client = new HttpClient();
            ...
            //wbx.DownloadFile(StateTaxationConstants.Henan2, @"D:/codeHenan.png");//Byte[] pageData = 
            Byte[] pageReponseData = wbx.UploadData(StateTaxationConstants.HenanPost, "POST", Encoding.UTF8.GetBytes($@"ACTION=getFpzw&
FPDM=141001620043&
FPHM=09767750&
NSRSBH=92410300MA3XG28Q9U&
rcode={vCode}"));
            Encoding enc2 = Encoding.GetEncoding("UTF-8");
            string reponse = enc2.GetString(pageReponseData);
            //CookieStr = wbx.ResponseHeaders["Set-Cookie"].ToString();
